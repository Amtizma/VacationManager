...
function createMarker(position, color, popupText) {
    ...
    iconElement.style.backgroundImage = "url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRiKV5WQKc4B2iNyqi3VdsKnbnkmOka-j1GXg&usqp=CAU)";
    markerContentElement.appendChild(iconElement);
    var popup = new tt.Popup({ offset: 30 }).setText(popupText);
    // add marker to map
    var marker = new tt.Marker({ element: markerElement, anchor: "bottom" }).setLngLat(position).setPopup(popup).addTo(map);
...
function createMarkerHotel(position, color, popupText) {
    ...
    var iconElement = document.createElement("div");
    iconElement.className = "marker-icon";
    iconElement.style.backgroundImage = "url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQ6zWQytBgNUC-6DZyltLg6zMqemc1f8W0dQ&usqp=CAU)";
    markerContentElement.appendChild(iconElement);
    var popup = new tt.Popup({ offset: 30 }).setText(popupText);
    // add marker to map
    var marker = new tt.Marker({ element: markerElement, anchor: "bottom" }).setLngLat(position).setPopup(popup).addTo(map);
