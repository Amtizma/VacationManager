...
function toggleTable() {
    ...
    function savehotels(response) {
        ...
        for (let i = 0; i < values.length; i++) {
            ...
            if (values[i] == "gross_amount_hotel_currency" && values[i + 1] == ":{" && values[i + 2] == "currency" && values[i + 3] == ":" && values[i + 4] == "EUR" && values[i + 5] == "," && values[i + 6] == "value")
                gross_price = values[i + 7].substring(1).replace("}", " ").replace(",", " ");
...
fetch(
    "https://booking-com.p.rapidapi.com/v1/hotels/search-by-coordinates?longitude=" +
        coordinates22[0] +
        "&locale=en-gb&adults_number=" +
        adults_number +
        "&filter_by_currency=EUR&checkout_date=" +
        checkout_date +
        "&units=metric&latitude=" +
        coordinates22[1] +
        "&order_by=popularity&room_number=" +
        room_number +
        "&checkin_date=" +
        checkin_date +
